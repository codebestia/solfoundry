...
def _log_event(session, wallet, event_type, amount, signature=None, rewards_amount=None, notes=None):
...
async def get_position(wallet: str) -> StakingPositionResponse:
    async with get_db_session() as session:
        ...
        if pos is None:
            return StakingPositionResponse(
                wallet_address=wallet, staked_amount=0.0, tier="none",
                apy=0.0, rep_boost=1.0, staked_at=None, last_reward_claim=None,
                rewards_earned=0.0, rewards_available=0.0,
                cooldown_started_at=None, cooldown_ends_at=None,
                cooldown_active=False, unstake_ready=False, unstake_amount=0.0,
            )
    ...
    async with get_db_session() as session:
        ...
        if pos is None:
            pos = StakingPositionTable(
                wallet_address=wallet, staked_amount=Decimal("0"),
                rewards_accrued=Decimal("0"), staked_at=now,
                last_reward_claim=now, updated_at=now,
            )
            session.add(pos)
        else:
...
async def get_history(wallet: str, limit: int = 50, offset: int = 0) -> StakingHistoryResponse:
    async with get_db_session() as session:
        ...
        items = [
            StakingEventResponse(
                id=str(row.id), wallet_address=row.wallet_address,
                event_type=row.event_type, amount=float(row.amount or 0),
...
async def get_platform_stats() -> StakingStats:
    async with get_db_session() as session:
        ...
        positions_result = await session.execute(
            select(StakingPositionTable.staked_amount).where(StakingPositionTable.staked_amount > 0)
        )
        ...
        return StakingStats(
            total_staked=total_staked, total_stakers=total_stakers,
            total_rewards_paid=total_rewards, avg_apy=avg_apy,
